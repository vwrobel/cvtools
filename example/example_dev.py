...
from cvtools import SelectionWindow, CompTracker, CompFilter, Processing, Vid
...
def example():
    ...
    orig_vid = Vid(vid_filepath)
    # The object you want to track (x, y , w, h are ratios between position and frame dimension)
    selection = SelectionWindow(0.56, 0.66, 0.085, 0.18, 5, (100, 100, 0), 'manual', 'Aouto', ['aouta'], orig_vid)
    ...
    orig_vid.get_samples(selection_list)
    ...
    filtered_vid_filepath = vid_filepath.replace('.mp4', '_filter_mhi.mp4')

    filtering = Processing(vid_filepath, filtered_vid_filepath, filter_mhi)
    filtering.run()
    ...
    tracked_vid_filepath = vid_filepath.replace('.mp4', '_tracker_meanshift_mhi.mp4')

    tracking = Processing(vid_filepath, tracked_vid_filepath, tracker)
    tracking.run()
